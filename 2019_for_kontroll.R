...
for (term in search_terms) {
  ...
  while (page_number < total_pages) {
    ...
    filter_values <- c("year:2019", "mediatype:aviser")
...
write.csv(result_df, "2019_conc_urn.csv", row.names = FALSE)


result_df <- read.csv("2019_conc_urn.csv")

##### Kombinering og legge til metadata
# Read the CSV files
result_df <- read.csv("2019_conc_urn.csv")
combined_df <- read.csv("setninger_labeled_MNLI_2019.csv", stringsAsFactors = FALSE)
...
write.csv(combined_URN_sortert, "2019_conc_renset.csv", row.names = FALSE)
