...
write.csv(result_df, "2023_conc_urn.csv", row.names = FALSE)

result_df <- read.csv("2023_conc_urn.csv")

##### Kombinering og legge til metadata
# Read the CSV files
result_df <- read.csv("2023_conc_urn.csv")
df1 <- read.csv("setninger_labeled_MNLI_2023.csv", stringsAsFactors = FALSE)
df2 <- read.csv("setninger_labeled_MNLI_2023_ekstra.csv", stringsAsFactors = FALSE)


# Merge the data frames
combined_df <- merge(df1, df2, by = "sentence", all.x = TRUE)
...
write.csv(combined_URN_sortert, "2023_conc_renset.csv", row.names = FALSE)
